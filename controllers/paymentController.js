...
// POST /api/payment/createSetupIntent  { userId }
//
// Save a card WITHOUT charging it — PaymentSheet in setup mode. Used before
// bookings that bill later (away mode's "valet reads the signs" path), so
// the eventual off-session charge always has a card to land on.
exports.createSetupIntent = async (req, res) => {
    ...
    try {
        ...
        const setupIntent = await stripe.setupIntents.create({
            ...
            usage: 'off_session',
            metadata: { userId: user._id.toString(), purpose: 'away_bill_later' },
        });
